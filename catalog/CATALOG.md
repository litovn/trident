...
- **Validation is enforced**: every entry is checked against `schema/catalog.schema.json` at load time (`ranker.Catalog.load` → fail fast). Bad enums (converter/scorer/owasp/capability/severity), missing required fields, malformed IDs, and dangling package→technique refs are rejected before anything runs.
