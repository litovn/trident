...
def _rules(top_n: int) -> str:
    return (
        "\nRules:\n"
        f"- If the operator's request clearly indicates a goal or attack surface, "
        f"respond with the {top_n} most relevant packages, best first, each with a "
        "one-sentence rationale tied to their request.\n"
        "- If the request is too vague to choose well, instead ask up to 3 short, "
        "concrete clarifying questions (e.g. is the target a bare chatbot or does it "
        "use RAG/tools? broad coverage or a specific goal like data exfiltration or "
        "jailbreak? recon-only or active attack?).\n"
        "- Prefer proposing once you have enough signal; do not over-question.\n"
